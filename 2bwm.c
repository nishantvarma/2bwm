 ...
 * Copyright (c) 2010, 2011, 2012 Michael Cardell Widerkrantz, mc at the domain
 * hack.org. Copyright (c) 2014, 2020 Patrick Louis, patrick at the domain
 * psychology dot wtf.
 *
...
int sigcode =
    0;  // Signal code. Non-zero if we've been interruped by a signal.
...
static const char *atomnames[NB_ATOMS][1] = {
    {"WM_DELETE_WINDOW"}, {"WM_CHANGE_STATE"}};
...
static void
maximize_helper(struct client *, uint16_t, uint16_t, uint16_t, uint16_t);
static void hide();
...
static void
movepointerback(const int16_t, const int16_t, const struct client *);
...
void
movepointerback(
    const int16_t startx, const int16_t starty, const struct client *client)
{
    if (startx > (0 - conf.borderwidth - 1) &&
        startx < (client->width + conf.borderwidth + 1) &&
        starty > (0 - conf.borderwidth - 1) &&
        starty < (client->height + conf.borderwidth + 1))
        xcb_warp_pointer(
            conn,
            XCB_NONE,
            client->id,
...
{  // Get EWWM hint so we might know what workspace window win should be
   // visible on. Returns either workspace, NET_WM_FIXED if this window should
   // be visible on all workspaces or TWOBWM_NOWS if we didn't find any hints.
...
check_name(struct client *client)
{
    ...
    for (i = 0; i < sizeof(ignore_names) / sizeof(__typeof__(*ignore_names));
         i++)
...
changeworkspace_helper(const uint32_t ws)
{
    ...
    pointer = xcb_query_pointer_reply(
        conn,
        xcb_query_pointer(conn, screen->root),
        0);
    if (pointer == NULL)
        setfocus(NULL);
    else
...
fitonscreen(struct client *client)
{
        ...
        client->x < mon_x || client->y < mon_y)
    {
        ...
        if (client->x > mon_x + mon_width)
            client->x =
                mon_x + mon_width - client->width - conf.borderwidth * 2;
        if (client->y > mon_y + mon_height)
            client->y =
                mon_y + mon_height - client->height - conf.borderwidth * 2;
        if (client->x < mon_x)
            client->x = mon_x;
        if (client->y < mon_y)
            client->y = mon_y;
    ...
    if (client->width + conf.borderwidth * 2 > mon_width)
    {
        ...
        client->x + client->width + conf.borderwidth * 2 > mon_x + mon_width)
    {
    ...
    if (client->height + conf.borderwidth * 2 > mon_height)
    {
        ...
        client->y + client->height + conf.borderwidth * 2 > mon_y + mon_height)
    {
        client->y =
            mon_y + mon_height - (client->height + conf.borderwidth * 2);
        willmove = true;
...
setupwin(xcb_window_t win)
{
    ...
    client->usercoord = client->vertmaxed = client->hormaxed = client->maxed =
        client->unkillable = client->fixed = client->ignore_borders =
            client->iconic = false;
    ...
    if (cookie.sequence > 0)
    {
        ...
        if (result)
        {
            ...
            if (parent)
            {
                ...
                client->y = parent->y + (parent->height / 2.0) -
                            (client->height / 2.0);
            }
        }
    }
...
grabkeys(void)
{
    ...
    unsigned int modifiers[] =
        {0, XCB_MOD_MASK_LOCK, numlockmask, numlockmask | XCB_MOD_MASK_LOCK};
...
setup_keyboard(void)
{
    ...
    for (i = 4; i < 8; i++)
    {
        ...
        {
            xcb_keycode_t keycode =
                modmap[i * reply->keycodes_per_modifier + j];
...
void
getoutputs(
    xcb_randr_output_t *outputs, const int len, xcb_timestamp_t timestamp)
{
    int i;
    int name_len;
    char *name;
    ...
    {
        if ((output =
                 xcb_randr_get_output_info_reply(conn, ocookie[i], NULL)) ==
            NULL)
            continue;
...
movelim(struct client *client)
{
    ...
    if (client->y - conf.borderwidth < mon_y || client->y < borders[2] + mon_y)
        ...
        client->y + client->height + (conf.borderwidth * 2) >
    ...
    if (client->x < borders[2] + mon_x)
        ...
        client->x + client->width + (conf.borderwidth * 2) >
...
focusnext_helper(bool arg)
{
    ...
    else
    {
        ...
        {
            // start from focus next and find first valid item on circular
            // list.
...
resizelim(struct client *client)
{
    ...
    if (client->x + client->width + conf.borderwidth > mon_x + mon_width)
        client->width =
            mon_width - ((client->x - mon_x) + conf.borderwidth * 2);
    if (client->y + client->height + conf.borderwidth > mon_y + mon_height)
        client->height =
            mon_height - ((client->y - mon_y) + conf.borderwidth * 2);
...
maxhalf(const Arg *arg)
{
    ...
    if (arg->i > 4)
    {
        if (arg->i > 6)
        {
            ...
            if (arg->i == TWOBWM_MAXHALF_FOLD_VERTICAL)
                ...
                focuswin->height =
                    focuswin->height * 2 + (2 * conf.borderwidth);
        }
        else
        {
            focuswin->y = mon_y;
            focuswin->height = mon_height - (conf.borderwidth * 2);
            focuswin->width =
                ((float)(mon_width) / 2) - (conf.borderwidth * 2);
    ...
    else
    {
        ...
        else
        {
            ...
            focuswin->width = mon_width - (conf.borderwidth * 2);
            focuswin->height =
                ((float)(mon_height) / 2) - (conf.borderwidth * 2);
...
teleport(const Arg *arg)
{
    ...
    if (arg->i == TWOBWM_TELEPORT_CENTER)
    { /* center */
        focuswin->x +=
            mon_width - (focuswin->width + conf.borderwidth * 2) + mon_x;
        focuswin->y +=
            mon_height - (focuswin->height + conf.borderwidth * 2) + mon_y;
        ...
        else
        {
            ...
            if (arg->i < 2)
                ...
                if (arg->i == TWOBWM_TELEPORT_CENTER_X)
                {
                    ...
                    focuswin->x +=
                        mon_width - (focuswin->width + conf.borderwidth * 2);
            else
            {
                /* bottom-right */
                focuswin->x +=
...
deletewin()
{
    ...
    cookie = xcb_icccm_get_wm_protocols_unchecked(
        conn,
        focuswin->id,
        ewmh->WM_PROTOCOLS);
...
changescreen(const Arg *arg)
{
    ...
    focuswin->x =
        focuswin->monitor->width * xpercentage + focuswin->monitor->x + 0.5;
    focuswin->y =
        focuswin->monitor->height * ypercentage + focuswin->monitor->y + 0.5;
