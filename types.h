...
{  // Everything we know about a window.
    ...
    uint16_t max_width, max_height, min_width, min_height, width_inc,
        height_inc, base_width, base_height;
    bool fixed, unkillable, vertmaxed, hormaxed, maxed, verthor,
        ignore_borders, iconic;
...
struct conf
{
    int8_t borderwidth;  // Do we draw borders for non-focused window? If so,
                         // how large?
    int8_t outer_border;  // The size of the outer border
    uint32_t focuscol, unfocuscol, fixedcol, unkillcol, empty_col,
        fixed_unkil_col, outer_border_col;
