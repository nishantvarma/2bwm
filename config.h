...
///---Modifiers---///
#define MOD \
    XCB_MOD_MASK_4 /* Super/Windows key  or check xmodmap(1) with -pm \
                      defined in /usr/include/xcb/xproto.h */
///--Speed---///
/* Move this many pixels when moving or resizing with keyboard unless the
 *window has hints saying otherwise. 0)move step slow   1)move step fast
 ...
 *  state 0x10, keycode 171 (keysym 0x1008ff17, XF86AudioNext), same_screen
...
static key keys[] = {
    ...
    DESKTOPCHANGE(XK_1, 0) DESKTOPCHANGE(XK_2, 1) DESKTOPCHANGE(XK_3, 2)
        DESKTOPCHANGE(XK_4, 3) DESKTOPCHANGE(XK_5, 4) DESKTOPCHANGE(XK_6, 5)
            DESKTOPCHANGE(XK_7, 6) DESKTOPCHANGE(XK_8, 7)
                DESKTOPCHANGE(XK_9, 8) DESKTOPCHANGE(XK_0, 9)};
